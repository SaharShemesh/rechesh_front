...
export function New_bid(props) {
     let current_date = new Date();
      ...
      let items = props.sell_Items.map(({desc,quantity,price},index) => ({desc,quantity:parseInt(quantity),price:parseFloat(price), getting_date:current_date.getFullYear() + "-" + month +"-" + day,key: index}));
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: 'price',
        key: 'price',
        render:(price) => <Input type="number" value={price} min="0" />
      },
      {
        title: 'מועד אספקה',
        ...
        return <Input type="date" value={date} onChange={
          (e) => {
             let new_items = sell_items.concat();
             new_items[index].getting_date = e.target.value;
             set_item(new_items);
          }
        } />
      ...
      <Form {...layout}>
        <Form.Item label=" שם ספק" >
          ...
          value={selected_provider.label}
            ...
            onChange={(valu) => {
            let provider_option = providers_collection.find(provider => provider.value == valu);
              if(!provider_option) provider_option = {value:-1, label:valu}; 
